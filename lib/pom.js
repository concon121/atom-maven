...
Pom.prototype.resolveAttributeValue = function (groupId, artifactId, version, pom, attribute) {
	...
	if (self._.isEmpty(version)) {
		self.resolveAttributeValueFromCollection(resultContainer, resultContainer.pom.dependencyManagement);
		self.resolveAttributeValueFromCollection(resultContainer, resultContainer.pom.dependencies);
		self.resolveAttributeValueFromParent(resultContainer);
	}
	return resultContainer.value || version;
};

Pom.prototype.resolveAttributeValueFromCollection = function (container, collection) {
	const self = this;
	var returned;
	if (collection) {
		returned = self.iterateDependencyCollection(collection, container.attribute, container.groupId, container.artifactId);
		if (returned) {
