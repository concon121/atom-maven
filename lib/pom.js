...
log4js.addAppender(log4js.appenders.file('pom.log'), 'pom');
const logger = log4js.getLogger('pom');
...
Pom.prototype.events = require('./events');
...
Pom.prototype.init = function (opts) {
	...
	if (opts) {
		...
		var buildEvent = function (suffix, callback) {
			logger.trace('Building event: ' + self.eventPrefix + suffix);
		...
		buildEvent('propertiesUpdated', () => {
			logger.trace('Loading dependency management');
			self.loadDependencyManagement(self);
		});

		buildEvent('dependencyManagementUpdated', () => {
			if (!self.hasDependencyManagement() || (self.dependencyManagement && (self.dependencyManagement.length === self.getDependencyManagement().length))) {
				logger.trace('Loading dependencies');
				self.loadDependencies(self);
			}
		});

		buildEvent('dependenciesUpdated', () => {
			if (!self.hasDependencies() || (self.dependencies && (self.dependencies.length === self.getDependencies().length))) {
				logger.trace('Loading cp');
		...
		buildEvent('classpathUpdated', () => {
			logger.trace('Finished loading cp');
			self.changing = false;
			...
			if (self.loadCompleteEvents && self.loadCompleteEvents.length > 0) {
				for (var loadCompleteEvent of self.loadCompleteEvents) {
					console.log("LOADING COMPLETE EVENT: ", self.pomPath);
					loadCompleteEvent(self);
				}
			}
		});
...
Pom.prototype.loadParent = function (pom) {
	...
	if (parentGAVT) {
		...
		promise.then((parent) => {
			...
			console.log("parent updated", pom.pomPath);
...
Pom.prototype.loadClasspath = function (pom) {
	...
	setTimeout(() => {
		function classpathContains(dependency) {
			...
			if (self._.isEmpty(self._.findWhere(classpath, condition))) {
				return false;
			} else {
				return true;
			}
...
Pom.prototype.resolveAttributeValue = function (groupId, artifactId, result, pom, attribute) {
	...
	if (self._.isEmpty(result)) {
		if (pom.dependencyManagement) {
			self.iterateDependencyCollection(pom.dependencyManagement, attribute, groupId, artifactId);
		}
		if (pom.dependencies) {
			self.iterateDependencyCollection(pom.dependencies, attribute, groupId, artifactId);
		}
		if (self._.isEmpty(result) && pom.dependants) {
			for (var dependant of pom.dependants) {
				returned = pom.resolveAttributeValue(groupId, artifactId, result, dependant, attribute);
