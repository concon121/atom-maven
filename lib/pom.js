...
Pom.prototype.init = function (opts) {
	...
	if (opts) {
		...
		function buildEvent(suffix, callback) {
			if (events.EventEmitter.listenerCount(self.events, self.eventPrefix + suffix) === 0) {
	...
	if (self._.isEmpty(result)) {
		...
		if (self._.isEmpty(result) && pom.dependants) {
			self.$.each(pom.dependants, (index, dependant) => {
				var returned = pom.resolveAttributeValue(groupId, artifactId, result, dependant, attribute);
		...
		if (self._.isEmpty(result) && pom.parent) {
			var returned = pom.resolveAttributeValue(groupId, artifactId, result, pom.parent, attribute);
			if (!self._.isEmpty(returned)) {
				result = returned;
			}
		}
