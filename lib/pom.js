class Pom {
	...
	constructor(opts, path) {
		...
		self.changing = false;
		if (!self.existsInRepo) self.existsInRepo = self.file.fileExists(path, false);

		if (path) self.pomPath = path;

		if (opts) {
			self.init(opts);
			self.registry.put(self.repoLocation, self);
		}

		self.reload(() => {
			console.log('reloading');
			if (opts) self.init(opts);
			else self.init(require('./maven-utils').getGAVT(self, self.project, 'pom'));
			self.registry.put(self.repoLocation, self);
		});
	}
...
Pom.prototype.init = function (opts) {
	...
	if (opts) {
		...
		if (!self.pomPath) self.pomPath = (opts.pomPath) ? opts.pomPath : self.repoLocation;
...
Pom.prototype.loadClasspath = function (pom) {
	...
	if (self.dependencies && self.dependencies.length > 0) {
		for (var projectDependency of self.dependencies) {
			self.classpath.push(projectDependency);
		}
	}
	...
	if (self.parent && self.parent.classpath) {
		for (var parentClasspathElement of self.parent.classpath) {
			if (!classpathContains(parentClasspathElement)) {
				self.classpath.push(parentClasspathElement);
			}
		}
	}
