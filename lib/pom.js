...
Pom.prototype.loadDependency = function (pom, dependency, managed, callback) {
	...
	if (managed) {
		...
		promise.then((real) => {
			...
			if (real.contains && !real.contains(real.dependants, pom)) {
...
Pom.prototype.addTransitiveDependenciesToClasspath = function (pom, classpath, factory, condition) {
	...
	if (pom.dependencies && pom.dependencies.length > 0) {
		for (var dependency of pom.dependencies) {
			if (dependency.classpath && notTestScoped(dependency)) {
				for (var transitiveClasspathElement of dependency.classpath) {
					if (!condition(transitiveClasspathElement) && notTestScoped(transitiveClasspathElement)) {
...
Pom.prototype.resolveAttributeValueFromDependencyManagement = function (container) {
	const self = this;
	var returned;
	if (container.attribute === 'version') console.log('Resolving from dependency management', container);
	if (container.pom.dependencyManagement) {
		...
		if (returned) {
			if (container.attribute === 'version') console.log('DEPENDENCY MANAGEMENT');
...
Pom.prototype.resolveAttributeValueFromDepenencies = function (container) {
	const self = this;
	var returned;
	if (container.attribute === 'version') console.log('Resolving from dependencies', container);
	if (container.pom.dependencies) {
		...
		if (returned) {
			if (container.attribute === 'version') console.log('DEPENDENCIES');
			container.value = returned;
		}
	}
};
...
Pom.prototype.resolveAttributeValueFromParent = function (container) {
	const self = this;
	var returned;
	if (container.attribute === 'version') console.log('Resolving from parent', container);
	if (self._.isEmpty(container.value) && container.pom.parent) {
		...
		if (returned) {
			if (container.attribute === 'version') console.log('PARENT');
			container.value = returned;
		}
	}
};
