var MavenUtils = function () {
	...
	return {
		...
		parseDependency: function (xmlDependency, pom, type) {
			...
			var result = {
				groupId: this.resolveProperty($(dependency.find("groupId")[0]).text(), pom),
				artifactId: this.resolveProperty($(dependency.find("artifactId")[0]).text(), pom),
				version: this.resolveProperty($(dependency.find("version")[0]).text(), pom),
				type: (dependency.find("type").length > 0) ? this.resolveProperty($(dependency.find("type")[0]).text(), pom) : type,
