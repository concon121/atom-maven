...
log4js.addAppender(log4js.appenders.file('logs/maven-utils.log'), 'maven-utils');

const logger = log4js.getLogger('maven-utils');
...
MavenUtils.prototype.getMavenGlobalSettings = function (path) {
	...
	if (mavenElems.length > 0) {
		...
		if (e && (e.indexOf('M2') >= 0 || e.indexOf('M3') >= 0)) {
			e = common.resolveEnvironmentVariable(e.replace(fsep + 'bin'));
		}
		settingsFileLocation = (e.endsWith(fsep + 'bin')) ? e.replace(fsep + 'bin', self.settings) : e.concat(self.settings);
		logger.trace('The settings file has been found: ' + settingsFileLocation);
	...
	fs.readFile(settingsFileLocation, 'utf8', (err, content) => {
		if (err) {
			...
			if (repo) {
				...
				logger.trace('The repo is: ' + self.repo);
			}
			callback();
		}
	});
...
MavenUtils.prototype.getPoms = function (callback) {
	...
	var pomPaths = [];
	self.setMavenRepo(() => {
		logger.trace('Scanning workspace for poms');
		atom.workspace.scan(/<project.*/g, (match) => {
			if (match.filePath.endsWith('pom.xml') || match.filePath.endsWith('.pom')) {
				logger.trace('Found pom file: ' + match.filePath);
				logger.trace('Maven is installed: ' + self.mavenIsInstalled);
	...
	var loadPomFromPath = function (index) {
		...
		if (pomPaths[index]) {
			console.log("LOADING: ", pomPaths[index]);
	...
	var interval = setInterval(() => {
		if (pomPaths.length > 0) {
			loadPomFromPath(index);
			if (index === pomPaths.length) {
