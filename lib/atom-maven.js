	...
	setMavenRepo() {
		...
		pathElems.every(function (elem) {
			if (elem.indexOf("maven") > -1) {
				var configLocation = (elem.endsWith("bin")) ? elem.replace("bin", settings) : elem.concat(settings);
				self.fs.readFile(configLocation, "utf8", (err, content) => {
					...
					console.log("The maven repo is located at: " + self.repo);
					return false;
				});
			}
			return true;
		});
		//$.each(pathElems, function (index, elem) {
