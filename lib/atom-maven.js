...
module.exports = {
  ...
  buildQueue: async.queue(function(command, callback) {
  ...
  existsInBuildQueue: function(command) {
    var self = this;
    console.log(self.buildQueue, self.buildQueue.workersList());
    return self.buildQueue.workersList().some(function(item, index) {
      if (item.data && item.data.path === command.path) {
        console.log(!(item.data.status === 'COMPLETE'));
        return !(item.data.status === 'COMPLETE');
      }
    });
  },
  addToBuildQueue: function(command) {
    var self = this;
    if (!self.existsInBuildQueue(self, command)) {
      console.log("Adding command", command)
      self.buildQueue.push(command);
    }
  },
  completeTask: function (command) {
    var self = this;
    console.log('Removing!', command)
    if (self.existsInBuildQueue(command)) {
      self.buildQueue.workersList().some(function (item) {
        if (item.data && item.data.path === command.path) {
          console.log("Setting as complete", item)
  ...
  failWithUI: function (message, match, callback, command) {
    var self = this;
    return () => {
      ui.error(message + match.filePath, null, null, match.filePath, null);
      self.completeTask(command);
      callback();
    };
  },
  ...
  setup: function () {
    ...
    atom.workspace.scan(/<project.*/g, (match) => {
      if (match.filePath.endsWith('pom.xml')) {
        ...
        maven.registerEvent('clean-failed', self.failWithUI(CLEAN_FAILED, match, buildFailed, command));
        maven.registerEvent('install', () => {
          ui.success(FINISHED_BUILDING + match.filePath);
          self.completeTask(_.clone(command));
        });
        maven.registerEvent('install-failed', self.failWithUI(BUILD_FAILED, match, buildFailed, command));
        ...
        self.addToBuildQueue(_.clone(command))
